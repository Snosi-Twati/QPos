...
class Desg extends CI_Model{
    ...
    function Create_From_Tabels($Tbl        ,
       ...
       //is_array($Creater) 
        ...
        foreach ($Creater as $key) {
            ...
            $this->InputFields($ViewFld,$key,$GetData);
   ...
   function InputFields($ViewFld,$key,$GetData) {
    ...
    function Create_From_Tabels_With_Ajax($Tbl        ,
        ...
        ////////////////////////////////////////////////////////////////////////
        $this->PostByAjax("#".$Tbl."Form", "#Form".$Tbl, 'Click', 'get', $ActionTo, "#".$Tbl."Data", $varJS, $DivReload);
        ////////////////////////////////////////////////////////////////////////
        
        echo "<div id=\"Form".$Tbl."\">";  
       ...
       //is_array($Creater) 
        ...
        if($btn!=FALSE&&!is_array($btn)){
            ...
            echo form_close();
            echo "</div>";
            $this->CallPostDataToURL("#".$Tbl."Form","#Form".$Tbl);
        }elseif($btn!=FALSE&&is_array($btn)) {
            $btndata = array(
                    "type"          =>  $btn["type"],
                    "id"            =>  $Tbl."Form",
...
//                var_dump($value['ServicePrice']);
    ...
    public function ExCol(&$Ary,$Title,$TBL,$ID,$TID ,$Url,$fa,$Color) {
            $Ary[]= array(
                'Title'     =>$Title    ,
                'ID'        =>$ID       ,
                'TID'       =>$TID       ,
    ...
    public function CreateSelectDropdown($key,$FldFrg,$GetData='',$AddClassName='myselect') {
        ...
        if(isset($FldFrg[$key->Field]['cond'])){
            ...
            echo "".$this->Create_dropdown_With_Cond($key->Field, 
                        ...
                        $FldFrg[$key->Field]['cond'],'class="form-control '.$AddClassName. $rn.'  " required="required"',$GetData)."<span class='input-group-addon'></span></div></div>"; 
        }else{
            ...
            echo "".$this->Create_dropdown( $key->Field,
                    ...
                    $FldFrg[$key->Field]['VFld'],
                    $FldFrg[$key->Field]['Fld'],
                    'class="form-control '.$AddClassName. $rn.'  " required="required"',$GetData)."<span class='input-group-addon'></span></div></div>";
        }      
        ...
        foreach ($Creater as $key) {
            if($Flds!=FALSE&&(isset($Flds[$key->Field])&&$Flds[$key->Field]==$key->Field))
            {
                   if(isset($RlFlds[$key->Field]))
                   {
                    ...
                    else echo "<th>".$this->LebalName($key->Field)."</th>";
                   }
                   else echo "<th>".$this->LebalName($key->Field)."</th>";
                ...
                //foreach ($ButtonAction as $Indx){
                        ...
                        foreach ($ButtonActionHover as  $BAvalue) {
                            ...
                            if($this->Pros->Get_JustValue_Filed_AQ_Multi_Cond($BAvalue['TBL'],$BAvalue['TID'],$Cnd)!=FALSE){
                                ...
                                ."</button>";}
                                else {"<button type=\"submit\" class=\"btn btn-primary fa ".$BAvalue['fa']." \" style=\"color:  ".$BAvalue['Color']."; \">".$BAvalue['TitleBtn'].
                                "</button>";}
